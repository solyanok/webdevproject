      ...
      {boards.length > 0 &&
        boards.map((board) => (
          <div className="board">
            ...
            <div className='icons'>
            <button type="button" onClick={()=>handleClickBoard(board._id)}><img className='icon' src={edit} alt='edit'></img></button>
            <button type="button" onClick={()=> handleDeleteBoard(board._id)} > <img className='icon' src={trash} alt='trash'></img> </button>
                  ...
                  task.boards_id === board._id ? (
                    <div className={`task ${task.completed ? 'completed' : ''}`}>
                      ...
                      <p>
                        ...
                        </p><button className='icon1' type="button" onClick={()=> handleDeleteTask(task._id)}><img className='icon' src={trash} alt='trash'></img></button>
                        </div>
                    
                      <button type="button" className='complete-btn' onClick={() => toggleTaskCompleted(task._id, !task.completed)}>
